...
function productShape(category: string, mode: string, accent: string) {
  if (category === "Posters") {
    return `
...
export async function GET(
  ...
  const mode = searchParams.get("mode") ?? "front";
  ...
  const film = product
    ...
    : null;
  const accent = film?.accentColor ?? "#ffae03";
  const title = product?.title ?? fallbackTitle;
  const category = product?.category ?? "Apparel";
  const collection = film?.title ?? "Portfolio Drop";
  const collectionLines = wrapText(collection, 16, 2);
  ...
  const svg = `
    <svg width="1080" height="1320" viewBox="0 0 1080 1320" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M0 0h1080v252L0 446z" fill="${accent}" opacity="0.16" />
      <path d="M1080 1320H0v-248l1080-174z" fill="${accent}" opacity="0.08" />
      ...
      ${productShape(category, mode, accent)}
